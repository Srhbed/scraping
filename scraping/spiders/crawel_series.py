...
class CrawlSeriesSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        ...
        duree=response.xpath('//li[@class="ipc-inline-list__item"]/text()')[1:].extract()
        if duree==[]:
        ...
        items['titre'] = response.xpath("//h1[@class='sc-b73cd867-0 eKrKux']/text()").extract()
        ...
        items['date']=response.xpath('//span[@class="sc-8c396aa2-2 itZqyK"]/text()').extract_first()
        ...
        items['langue']=response.xpath('/html/body/div[2]/main/div/section[1]/div/section/div/div[1]/section/div[2]/ul/li[4]/div/ul/li/a/text()')[1].extract()



        

        yield items          # 'titre': titre,
            # 'score':score,
            # 'genre':genre,
            # 'date':date,
            # 'duree':duree,
